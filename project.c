...
void wordQuiz(void)                //단어 맞추기 게임(김도균)
{
	...
	if (output_select == 1)
		ascendingOrderWords(buffer, 100, fp);  //단어 오름차순 출력.
	else if (output_select == 2)          //노드 무작위 출력
		randomWords(buffer, 100, fp);     //무작위 순서로 출력.
...
void ascendingOrderWords(char * buffer, int buffersize, FILE * fp)
{
	...
	head = NULL;             //532 533 추가하니 정상동작됨..........(1204) ??
	cursor = NULL;
...
void hangman() { //행맨 함수
	...
	//행맨 함수 지역변수
	int hangman_count = 1; //행맨 횟수 
	int try_num = 1;	   //시도 횟수
	int correct_count = 0; //한 글자씩 비교하며 맞은 횟수
	int loop_count;		   //반복 횟수
	_Bool answer_state = 0;//최종 정답인지 아닌지 검사를 위한 변수
	char input_word;       //입력 받을 한 글자
	char eng_word_line[20];//행맨의 쓰일 영 문자열
	char kor_word[20];     //파일에서 받은 뜻 문자열
	char dic_eng[20];      //파일에서 받은 영 문자열
	char buffer[50];       //문자열 단위로 가져와 받기위한 문자열 

	//파일에서 해당 파일을 불러와서, 해당 구조체단위로 가공할 배열에 복사
	FILE * ffp = DayDicLoad();
	if (ffp == NULL)
		return;
	randomWords(buffer, 100, ffp);
	...
	int word_length = hangman_word(eng_word_line);//hangman_word() 함수를 호출
	char cmd;
	...
	for (loop_count = 1; loop_count <= 20; loop_count++) {
		...
		if (hangman_count>6)
			break;
		scanf("%c", &input_word); //단어 한 글자 입력 부분
		
		for (int k = 0; k < word_length; k++) { //반복문으로 문자열내에서 입력한 글자가 있는지 확인
			if (input_word == dic_eng[k]) {
				eng_word_line[k] = input_word;//만약 정답이라면,'_' 문자를 해당 문자로 대치
				answer_state = 1;             //부분 정답횟수(참)

			}
			if (eng_word_line[k] != '_') {
				correct_count++;             //누적 정답횟수 결정 부분
			}
		}
		getchar();
		//게임 승리 부분
		if (correct_count == word_length) {//누적 정답횟수와 문자열 길이가 같음->정답
			printf("   ##########################\n");
		...
		if (answer_state == 1) {//부분 정답일 시
			try_num++;			//시도 횟수 증가
			answer_state = 0;	//부분정답 초기화
		}
		else {					//부분 정답이 아닐 시
			hangman_count++;    //행맨 횟수 증가
			try_num++;          //시도 횟수 증가
			answer_state = 0;   //부분 정답 초기화
		}
	}
	//게임 종료 부분
