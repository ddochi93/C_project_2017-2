...
} word_list;               // 영어단어1개 , 한글뜻3개, linked list를 만들기 위한 포인터를 포함한 구조체 선언.
typedef word_list Node;
Node * head = NULL;          
Node * cursor = NULL;      // Node들을 연결하기위해 가장 처음 Node를 가리키기위한 head와 현재 Node를 가리키기 위한 포인터 선언.

void wordQuiz();    // 영어단어 맞추기 함수 (김도균)
void flashCard();    // 플래쉬 카드 함수 (김도균)
void hangman();      // 행맨 게임 함수(장종현)
void manageWord();   // 단어장 관리 함수(이진영)
void addFile();
void newWords();
void addWords(int num, bool is_new_file);
void showDic();
void ShowDicFiles();
void myflush();      //입력버퍼 비우는 함수(김도균)
void gotoxy(int x, int y);  // 커서 이동 함수(김도균)
void ascendingOrderWords(char * buffer, int buffersize, FILE * fp); //오름차순으로 node정렬 함수(김도균)
void randomWords(char *buffer, int buffersize, FILE *fp);   //무작위로 node 정렬 함수(김도균)
FILE * DayDicLoad();             //입력한 일차의 단어장을 로드하는 함수. (김도균)
...
void myFree(Node *);            //각 게임에서  할당된 newNode들을 free시키는 함수. (김도균)
...
void flashCard()
{
	...
	while (1)
	{
		...
		fflush(stdout);             //출력전에 sleep이 되서 화면에 안나오기때문에 출력버퍼를 비워줌.
		sleep(t);
		system("clear");
		gotoxy(60,25);

		for( i=0 ; i<3 ; i++)         //한글뜻 3개 출력
		{
...
void myFree(Node * head)   //재귀함수 통해 모든노드의 동적할당을 해제(김도균)
{
    if(head -> next != NULL)
        myFree(head->next);
    free(head);
